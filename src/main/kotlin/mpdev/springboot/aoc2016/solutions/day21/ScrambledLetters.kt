...
class ScrambledLetters(inputDataReader: InputDataReader): PuzzleSolver(inputDataReader, 21) {
    ...
    override fun solvePart1(): String {
        ...
        instructionList.forEach { instr ->
            val previous = result
            result = instr.instruction.execute(result, instr.param1, instr.param2)
            if (result.size != previous.size)
                throw AocException("wrong result: prev.string: [$previous], instuction: $instr, result: [$result]")
        }
...
fun List<Char>.rotateBasedOnPositionOfLetterLeft(a: String): List<Char> {
    val l = CircularList(this.toMutableList())
...
enum class Instruction(val value: String, val execute: (List<Char>, String, String) -> List<Char>) {
    SWP_POS("swap position", { l, a, b -> l.swapPosition(a, b) }),
    SWP_LTR("swap letter", { l, a, b -> l.swapLetter(a, b) }),
    REV_POS("reverse positions", { l, a, b -> l.reversePositions(a, b) }),
    ROT_RGT("rotate right", { l, a, _ -> l.rotateRight(a) }),
    ROT_LFT("rotate left", { l, a, _ -> l.rotateLeft(a) }),
    MOV_POS("move position", { l, a, b -> l.movePosition(a, b) }),
    ROT_PLR("rotate based on position of letter", { l, a, _ -> l.rotateBasedOnPositionOfLetterLeft(a) }),
    NOP("NO_OPERATION", { _, _, _ -> emptyList() });
