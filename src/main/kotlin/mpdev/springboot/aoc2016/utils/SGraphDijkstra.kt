...
inline fun <reified T> SGraph<T>.dijkstra(from: T, isAtEnd: (T) -> Boolean):  MinCostPath<T> {
    ...
    SGraph.aStarAlgorithm = false   // ensure dijkstra is used
    var iterations = 0
...
data class SGraphPathNode<T>(val id: T?, var costFromStart: Int, var updatedBy: T? = null):
    ...
    var estTotalCostToEnd: Int = Int.MAX_VALUE    // used in A* algorithm only
    override fun compareTo(other: SGraphPathNode<T>): Int {
        return if (!SGraph.aStarAlgorithm)    // in Dijkstra min cost criteria is based on cost from start to this node
